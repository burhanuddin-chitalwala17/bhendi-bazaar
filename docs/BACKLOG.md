...
- **Storefront reads are uncached, and page-level ISR cannot fix it** — PR-82 turned prefetch off at 24 storefront link sites, which removed the speculative load but not the per-request cost: a category page is still roughly three sequential round trips, and it renders identical HTML for every visitor. Nothing under `src/app/(main)` reads `cookies()`, `headers()` or the session on the server — the auth-aware chrome is all `"use client"` on `useSession()` — so there is nothing per-user in the render to justify repeating it.
