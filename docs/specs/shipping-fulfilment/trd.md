...
- **D5** — Retries are bounded and idempotent, keyed so a retry cannot produce a second parcel. `server/utils/retry.ts` exists but retrying a placeholder proved nothing; a real booking needs idempotency, not just repetition.
