...
| `bcryptjs` | `^3.0.3` | Password hashing. Cost 10 | `server/services/passwordService.ts` | 2026-08-03 |
| `razorpay` | `^2.9.5` | Payment gateway SDK. **Note:** `server/repositories/razorpayRepository.ts` calls the REST API with `fetch` directly rather than using this SDK; one of the two should go | payments domain | 2026-08-03 |
...
| `resend` | `^6.6.0` | Transactional email | `server/services/emailService.ts` | 2026-08-03 |
