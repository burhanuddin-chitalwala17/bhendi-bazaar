...
- **No mock or placeholder implementation in this tree.** A test double lives under `tests/`, where application code cannot import it. A stub inside `server/` is reachable in production, and reachability is the whole problem — this is why booking currently produces placeholder tracking data.
...
- **Every external call has a timeout and a bounded, idempotent retry.** A retry that can produce a second parcel is worse than a failure. `server/utils/retry.ts` provides the mechanism; idempotency is the caller's responsibility.
